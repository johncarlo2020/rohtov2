<x-app-layout>
    @php
        ...
        $rewards = [
            ...
            7 => ['Giant Gashapon Surprise', 95, 4],
            8 => ['Exclusive Merchandise', 87, -6],
        ];
    ...
    <main id="map-page" class="content-box" data-view="journey">
        <div class="map-shell">
                    ...
                    <div data-map-layer="{{ $view }}" @if ($view === 'rewards') hidden @endif>
                        @foreach ($locations as $id => [$label, $left, $top])
                            ...
                            <a class="journey-pin {{ $view === 'rewards' ? 'reward-pin' : '' }}"
                                @if (!in_array($id, $lockedStationIds)) href="{{ route('station', $id) }}" @else aria-disabled="true" tabindex="-1" @endif
                                data-location="{{ $id }}"
                                ...
                                <svg class="journey-pin-shape" viewBox="0 0 30 48" aria-hidden="true" focusable="false">
                                    <path
                                        d="M15 1C7.27 1 1 7.27 1 15c0 6 3.8 11.3 9.5 13.3L15 46l4.5-17.7C25.2 26.3 29 21 29 15 29 7.27 22.73 1 15 1Z" />
                                </svg>
                                <span aria-hidden="true">
                                    @if ($isCompleted)
                                        <i
                                            class="fa-solid fa-check"></i>@else{{ $view === 'rewards' ? $loop->iteration : $id }}
                                    @endif
                                </span>
                            </a>
                        @endforeach
                        @if ($view === 'rewards' && !$iscardApplied)
                            @foreach ($booths as [$left, $top])
                                <button type="button" class="card-map-pin" data-location="booth"
                                    style="--pin-left: {{ $left }}%; --pin-top: {{ $top }}%"
                                    aria-label="Card Sales Booth" aria-pressed="false">
            ...
            <div class="map-legend-wrap">
                <section class="map-legend" data-map-panel="journey" aria-labelledby="journey-title">
                    ...
                    <div class="legend-scroll" tabindex="0" aria-label="Journey locations">
                        ...
                        @foreach ($journey as $id => [$label])
                            @php($isCompleted = in_array($id, $completedStationIds))
                            <button type="button" class="legend-location" data-location="{{ $id }}"
                                data-station-url="{{ route('station', $id) }}" @disabled(in_array($id, $lockedStationIds))>
                                <span class="legend-number">
                                    @if ($isCompleted)
                                        <i class="fa-solid fa-check"></i>@else{{ $id }}
                                    @endif
                                </span>
                                <span>{{ $label }}</span>
                                @if (in_array($id, $lockedStationIds))
                                    <i class="fa-solid fa-lock station-lock" aria-label="Locked"></i>
                                @endif
                            </button>
                        @endforeach
                        <button type="button" class="legend-banner legend-banner-card" data-show-rewards
                ...
                <button type="button" class="map-view-toggle" aria-label="Show more rewards"><span
                        aria-hidden="true"></span></button>
            </div>
        </div>
    </main>
    <dialog id="card-apply-dialog" class="card-apply-dialog" aria-labelledby="card-apply-message"
        data-scan-url="{{ route('card-application.scan') }}">
